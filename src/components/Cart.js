...
import { UpIcon, DownIcon, TrashIcon } from './icon';

const Cart = () => {
  const [cartItems, setCartItems] = useState([]);
  const navigate = useNavigate();
  const { getCartItems, clearBasket, increaseQuantity, decreaseQuantity, removeProduct } = useContext(CartContext);

  console.log(cartItems, 'main checked');

  useEffect(() => {
    console.log(getCartItems(), 'is useEffect called here');
